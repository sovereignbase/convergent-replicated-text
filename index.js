...
function __create(snapshot2) {
  ...
  for (const valueEntry of snapshot2.values) {
    const linkedListEntry = materializeSnapshotEntry(valueEntry, crListReplica)
    ...
    switch (mode) {
      case 'overwrite': {
        if (listIndex === crListReplica.size) {
          ...
          void linkEntryBetween(crListReplica.cursor, linkedListEntry, void 0)
          void attachEntryToIndexes(crListReplica, linkedListEntry, delta)
          crListReplica.cursor = linkedListEntry
          crListReplica.cursorIndex = linkedListEntry.index
          void crListReplica.index?.set(linkedListEntry.index, linkedListEntry)
...
var CRList = class {
  ...
  snapshot() {
    ...
    if (snapshot2)
      void dispatchCRListEvent(this.eventTarget, 'snapshot', snapshot2)
  }
  /**
...
var CRText = class {
  ...
  snapshot() {
    const snapshot2 = __snapshot(this.state)
    if (snapshot2)
      void dispatchCRTextEvent(this.eventTarget, 'snapshot', snapshot2)
  }
  /**
